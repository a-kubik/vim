    ...
    DWORD  cbToWrite)
{
    ...
    if (enc_codepage >= 0)
    {
	static WCHAR	*unicodebuf;
	static int	unibuflen;
	int		length;
	DWORD		n, cchwritten, cells;
	
	length = MultiByteToWideChar((UINT)enc_codepage, 0, (LPCSTR)pchBuf, cbToWrite, 0, 0);
	...
	MultiByteToWideChar((UINT)enc_codepage, 0, (LPCSTR)pchBuf, cbToWrite,
